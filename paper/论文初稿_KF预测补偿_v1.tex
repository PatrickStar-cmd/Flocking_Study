...
现有多智能体协同控制方法大致可分为集中式控制、领航者--跟随者控制、行为法与虚拟结构控制，以及分布式一致性、优化与集群控制。集中式控制便于获得全局协调结果，但通信和计算负担大且存在单点失效风险；领航者--跟随者控制结构简单，但依赖领航者可靠性；行为法和虚拟结构控制能够描述局部行为或维持队形，但参数调节或结构适应性通常受限；分布式一致性方法仅依靠局部交互即可获得良好的可扩展性和鲁棒性\upcite{OlfatiSaber2006}，却更易受感知退化、通信受限和拓扑变化影响。为提升不确定环境下的适应性，相关研究进一步采用不确定性感知的分布式集群控制和预测控制，但此类方法通常依赖距离、位置或完整状态信息。

仿生 flocking 因仅依赖局部邻居信息即可产生群体有序运动、适应动态环境和规模扩展，成为无人集群分布式控制的重要范式。其早期研究通过局部对齐、吸引和排斥规则揭示了自驱动粒子的有序相变\upcite{Vicsek1995}；随后，flocking 被推广到连续时间多智能体系统，并通过一致性理论建立了分布式队形控制方法\upcite{OlfatiSaber2006}。在此基础上，异中心神经场 flocking 将邻居方位编码为世界参考系中的活动峰，使群体方向不再依赖统一的机体参考系\upcite{Salahshour2025}。然而，该模型对社会感知的连续性要求及其在间歇方位观测下的输入重构问题仍未得到系统解决。

然而，异中心神经场 flocking 的理想动力学通常假定邻居社会输入连续、可靠且无延迟，实际系统中的遮挡、有限视场、链路中断和测量噪声会造成方位漏检或长时间缺失。直接删除输入会削弱群体方向信息；观测缺失后继续使用最后一次方位，会让神经场持续接收已经过期的方向信息，形成“幽灵邻居输入”，从而使活动峰扩散、方向表征失同步，并引起全局有序度（global order, GO）和局部有序度（local order, LO）退化。现有 Kalman 滤波和纯方位跟踪方法主要恢复位置、速度或一般状态，难以直接满足仅方位观测下周期神经场输入的建模要求，因此需要针对神经场输入构造专用的方位预测与不确定性传播接口。

将 Kalman 预测与异中心神经场直接结合仍存在三个待解决的科学问题：其一，仅方位观测不能直接恢复邻居距离和二维位置，需要构造与局部角运动一致的因果预测状态；其二，滤波器协方差不能仅停留在估计误差内部，需要建立其到圆周神经场输入的可解释映射，并量化连续缺失期间方向信息的保持能力；其三，闭环收益容易与“保留最后一次方位”或单纯删除输入的效果混淆，需要可辨识困难场景的配对实验来分离轨迹记忆、均值外推和不确定性传播的作用。因此，本文将创新工作分为框架、理论和仿真三个层次。

本文围绕以下问题展开：
\begin{enumerate}[label=(\arabic*)]
  \item 在异中心神经场模型中，社会输入数量、总幅值和方向覆盖如何影响持续集群运动？
  \item 瞬时漏检与连续观测缺失是否会产生不同的失稳过程？
  \item 方位 Kalman 预测及其协方差传播能否在有限缺失窗口内延缓集群退化，并减少对新鲜社会输入的需求？
\end{enumerate}

本文的创新点分为以下三个层次：
\begin{enumerate}[label=(\arabic*)]
  \item 框架层创新：建立间歇仅方位观测下 Kalman 预测与异中心神经场的耦合框架。该框架不假设可以恢复邻居的二维位置或速度，而是以方位--方位角速度二状态为预测对象，将方位预测均值与后验协方差同时转化为神经场的周期社会输入，从而在观测缺失期间维持已跟踪邻居的方向表征。
  \item 理论层创新：解决仅方位状态建模和不确定性向周期神经场映射两个核心理论问题。本文建立局部展开角与圆周方位的统一递推，并推导连续缺失下方位方差的时间增长规律、圆周方向谐波的保留比例、方向信息保持时域以及协方差卷积输入的条件均方最优性和误差界，说明协方差卷积如何抑制过期方向信息并拓宽方向核，同时明确其成立条件与适用边界。
  \item 仿真层创新：针对直线收敛状态下已有航向惯性可能掩盖观测退化、预测增益又容易与 hold-last 的轨迹记忆混淆的问题，设计包含 fresh-drop、hold-last、KF-mean、KF-uncertainty 和 KF-GM-uncertainty 的配对闭环实验，并增加遮挡前受控转向场景。配对统计能够分解轨迹记忆、均值外推和协方差传播的作用，受控转向场景能够放大连续缺失导致的退化；实验结果最终明确了 Kalman 补偿在群体有序度、凝聚结构和方位预测方面的有效窗口，同时表明协方差卷积不能替代概率校准，也不能被视为碰撞安全保证。
\end{enumerate}
...
\section{间歇方位观测下的预测补偿理论分析}

本章在局部线性高斯方位模型下，将协方差传播与圆周核卷积联系起来，给出方向信息衰减规律，并进一步说明协方差感知输入在给定后验模型下的条件均方最优性。上述结论分别用于解释预测补偿的时间限制和协方差进入神经场的数学依据。

\subsection{基本假设与连续缺失下的方差传播}
设最后一次更新后的协方差为
\begin{equation}
 P_0=\begin{bmatrix}p_{11}&p_{12}\\p_{12}&p_{22}\end{bmatrix}.
\end{equation}
间歇观测下 Kalman 滤波的协方差增长与估计稳定性已有一般性理论分析\upcite{Sinopoli2004}。本节进一步把该协方差传播与圆周神经场输入联系起来。

连续 $b$ 步缺失，令 $T=b\Delta t$，则
\begin{equation}
 P(T)=F_TP_0F_T^\top+q_\omega\begin{bmatrix}T^3/3&T^2/2\\T^2/2&T\end{bmatrix},
 \quad F_T=\begin{bmatrix}1&T\\0&1\end{bmatrix},
\end{equation}
从而
\begin{equation}
 \boxed{V(T)=p_{11}+2Tp_{12}+T^2p_{22}+q_\omega T^3/3.}
\end{equation}
式中，$p_{11}$、$p_{12}$ 和 $p_{22}$ 是最后一次更新后协方差矩阵的元素，$T=b\Delta t$ 是连续缺失时长，$q_\omega$ 是角加速度噪声谱强度。该式由协方差递推和连续白角加速度噪声积分直接得到。由于交叉项 $p_{12}$ 可能为负，$V(T)$ 不一定在每一个短时间步都单调增大；保守上界可写为
\begin{equation}
 V(T)\le p_{11}+2T|p_{12}|+T^2p_{22}+q_\omega T^3/3.
\end{equation}

\subsection{方向信息保持时域}
若要求一阶方向信息至少保留 $\gamma_{\min}$，则由式（\theequation）可得充分条件
\begin{equation}
 V(T)\le -2\ln\gamma_{\min}.
\end{equation}
该条件定义的是预测方向信息保持窗口，不是集群安全时间或全局稳定时间。它依赖后验协方差、过程噪声和指定的容许信息损失比例。

\subsection{协方差感知输入的条件均方最优性}
对相同的已跟踪邻居集合，定义理想社会输入向量 $J_i$，其中 $\mathcal Y_i$ 表示智能体 $i$ 的可用观测历史。若所采用的方位后验模型为 wrapped normal 分布，则协方差卷积输入满足 $\bar J_i=\mathbb E[J_i\mid\mathcal Y_i]$。任意基于相同观测历史的估计 $\widetilde J_i$ 有
\begin{equation}
 \mathbb E[\|J_i-\widetilde J_i\|_2^2\mid\mathcal Y_i]
 =\mathbb E[\|J_i-\bar J_i\|_2^2\mid\mathcal Y_i]
 +\|\bar J_i-\widetilde J_i\|_2^2.
\end{equation}
因此，周期卷积输入在所采用后验模型下最小化社会输入重建的条件均方误差。该结论只针对相同轨迹、相同观测历史和相同已跟踪邻居集合，不能直接推出更高的 GO 或更低的危险接近时间占比。

\subsection{均值代入输入误差界}
当 $0<\sigma_s\le\pi$ 时，圆周核满足 Lipschitz 界
\begin{equation}
 |g_\sigma(\alpha-\mu)-g_\sigma(\alpha-\theta)|
 \le \frac{e^{-1/2}}{\sigma_s}d_{\mathbb S^1}(\mu,\theta).
\end{equation}
因此，对于相同已跟踪集合，均值代入输入的条件期望误差满足
\begin{equation}
 \mathbb E\left[\|I^{\mathrm{mean}}_i-I^{\mathrm{true}}_i\|_\infty\mid\mathcal Y_i\right]
 \le h_0\frac{e^{-1/2}}{\sigma_s}\sum_j\sqrt{V_{ij}}.
\end{equation}
该式说明方位协方差越大，均值输入与真实输入之间的可控误差上界越宽；若存在从未跟踪的真实邻居，还需额外加入其未建模输入项。

\subsection{理论结果的作用与适用边界}

上述结果把协方差传播、圆周 Fourier 分析和条件期望投影用于方位预测与神经场输入之间的接口。前者支持按不确定性设定预测信息保持窗口，后者支持将协方差作为输入核展宽的依据，而不仅用于滤波器内部更新。

两项结论均需在其模型条件下解释。常角速度模型失配、局部角度分支失效或协方差校准不足时，计算得到的 $V$ 未必反映真实误差，方向信息窗口和均方最优性也不构成真实系统的无条件保证。对于阻尼角速度预测器，只要所采用的方位后验仍为包裹正态分布，方向谐波衰减和条件均方最优性仍然适用，但方差必须由阻尼模型的协方差递推计算。社会输入重建误差经过非线性神经场与运动闭环后，也不必然转化为更高的群体有序度或更小的凝聚距离偏差。
